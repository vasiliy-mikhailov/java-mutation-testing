...
Run **every** build/test/analysis command under the chosen JDK — via `JAVA_HOME`, your harness's
JDK-selection, or by executing in the matching JDK container/image. Report the detected version so
downstream steps reproduce the same environment.
